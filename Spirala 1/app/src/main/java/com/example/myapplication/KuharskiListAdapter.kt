...
class KuharskiListAdapter(private var biljke : List<Biljka>) : RecyclerView.Adapter<KuharskiListAdapter.BiljkaViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BiljkaViewHolder, position: Int) {
        ...
        holder.biljkaOkus.text = biljke[position].profilOkusa.toString()

        holder.biljkaKorist1.text = biljke[position].jela[0].toString()
        if(1<biljke[position].jela.size){
            holder.biljkaKorist2.text = biljke[position].jela[1].toString()
        }else {
            holder.biljkaKorist2.text = "" }
        if(2<biljke[position].jela.size) {
            holder.biljkaKorist3.text = biljke[position].jela[2].toString()
        }else {
